...
def find_importance():
    ...
    for name in model_names:
            ...
            importance = pi(model, package[2], package[5], n_repeats = 20, random_state=42)
            #create dataframe that housed the mean results and the stand_deviattions
            importance_df = pd.DataFrame({'importance_mean': importance['importances_mean'],'importance_std': importance['importances_std']}, index= model.feature_names_in_)
